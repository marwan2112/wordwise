// app.js - التطبيق الكامل مع جميع التحسينات المطلوبة
class App {
    constructor() {
        ...
        try {
            ...
            try {
                ...
                if (error.code === 'auth/user-not-found' || error.code === 'auth/invalid-credential') {
                    ...
                    this.render();
                    this.showCustomModal('success', '🎉', this.t(`مرحباً ${name}! تم إنشاء حسابك بنجاح. تم منحك 100 لؤلؤة 💎`, `Welcome ${name}! Your account has been created. You got 100 pearls 💎`));
                    this.isProcessingAuth = false;
                    return;
                } else throw error;
            }
            ...
            this.currentPage = 'home';
            this.render();
            // لا نعرض رسالة "تم تسجيل الدخول بنجاح"
        } catch (error) {
    ...
    getCurrentAdReward() {
        ...
        for (let i = 0; i < this.adRewardLevels.length; i++) {
            if (dailyData.count < this.adRewardLevels[i].adsNeeded) {
                return { adsNeeded: this.adRewardLevels[i].adsNeeded, reward: this.adRewardLevels[i].reward, currentCount: dailyData.count };
            }
        }
        return null;
    }
    ...
    watchAdsForCoins() {
        ...
        this.showAd('rewarded', (success) => {
            if (success) {
                ...
                this.showCustomModal('success', '🎉', this.t(`تهانينا! حصلت على ${rewardInfo.reward} لؤلؤة. (الإعلان ${dailyData.count}/${rewardInfo.adsNeeded} من هذا المستوى)`, `Congratulations! You earned ${rewardInfo.reward} pearls. (Ad ${dailyData.count}/${rewardInfo.adsNeeded} of this level)`));
                this.saveUserData();
                this.render();
            }
        });
    ...
    showCustomModal(type, icon, message, onClose = null) {
        const modalDiv = document.createElement('div');
        modalDiv.className = 'modal-overlay';
        modalDiv.onclick = (e) => { if (e.target === modalDiv) { modalDiv.remove(); if (onClose) onClose(); } };
        ...
        confirmBtn.onclick = () => { modalDiv.remove(); if (onClose) onClose(); };
    }
    ...
    // دوال التطبيق الأساسية (بطاقات، استماع، كتابة، ترتيب، ملء فراغ، اختبارات، دروس، إلخ)
    ...
    updateProfile() {
        ...
        if (newName) { this.userProfile.name = newName; if (this.userData && this.currentUser) { this.userData.name = newName; updateProfile(this.currentUser, { displayName: newName }).catch(console.error); } }
        if (newAge) this.userProfile.age = newAge;
        if (newPassword && this.userData && this.currentUser) { updatePassword(this.currentUser, newPassword).catch(console.error); }
        if (imageFile) { const reader = new FileReader(); reader.onload = (e) => { this.userProfile.image = e.target.result; this.saveUserData(); this.render(); }; reader.readAsDataURL(imageFile); }
        else { this.saveUserData(); this.render(); }
    }

    getEnglishLevel() { return this.userProfile.level || 'A1'; }

    showProfileInfo() { this.showProfileInfoModal = true; this.render(); }
    showChangePasswordForm() { this.showChangePasswordForm = true; this.render(); }
    cancelChangePassword() { this.showChangePasswordForm = false; this.render(); }

    changePassword() {
        const oldPass = document.getElementById('oldPassword')?.value;
        const newPass = document.getElementById('newPassword')?.value;
        const confirmPass = document.getElementById('confirmPassword')?.value;
        if (!oldPass || !newPass || !confirmPass) { this.showCustomModal('error', '❌', this.t('الرجاء ملء جميع الحقول', 'Please fill all fields')); return; }
        if (newPass !== confirmPass) { this.showCustomModal('error', '❌', this.t('كلمة المرور الجديدة غير متطابقة', 'New passwords do not match')); return; }
        ...
        if (user) {
            updatePassword(user, newPass).then(() => {
                ...
                this.showProfileInfoModal = false;
                this.render();
            }).catch(err => { this.showCustomModal('error', '❌', this.t('فشل تغيير كلمة المرور: ' + err.message, 'Failed to change password: ' + err.message)); });
        }
    }
    ...
    getAdaptiveQuestion() {
        ...
        if (!levelQuestions || levelQuestions.length === 0) return window.placementBank['A1'][0];
        ...
        this.currentPlacementDetails.push({ level: this.currentDifficulty, question: selected.q, options: selected.options || [selected.a, selected.b, selected.c, selected.d].filter(o => o !== undefined), correct: correctAnswer, selected: null, isCorrect: null });
        return selected;
    }
    ...
    handlePlacement(selected, correct, btnElement) {
        ...
        const allOptions = document.querySelectorAll('.quiz-opt-btn');
        allOptions.forEach(btn => { btn.disabled = true; btn.classList.remove('correct-answer', 'wrong-answer', 'other-option'); const btnParam = btn.dataset.param ? btn.dataset.param.trim().toLowerCase() : ''; if (btnParam === correctTrim) btn.classList.add('correct-answer'); else if (btnParam === selectedTrim && !isCorrect) btn.classList.add('wrong-answer'); else btn.classList.add('other-option'); });
        if (this.currentPlacementDetails.length > 0) { const last = this.currentPlacementDetails[this.currentPlacementDetails.length - 1]; last.selected = selected; last.isCorrect = isCorrect; }
        setTimeout(() => {
            const levels = ['A1', 'A2', 'B1', 'B2', 'C1', 'C2'];
            let idx = levels.indexOf(this.currentDifficulty);
            if (idx === -1) idx = 0;
            if (isCorrect && idx < levels.length - 1) { this.currentDifficulty = levels[idx + 1]; this.placementCorrectStreak = 0; }
            else if (!isCorrect && idx > 0) { this.currentDifficulty = levels[idx - 1]; this.placementCorrectStreak = 0; }
            this.placementStep++;
            if (this.placementStep >= 35) {
                let finalLevel = 'A1';
                for (let i = 0; i < this.currentPlacementDetails.length; i++) { if (this.currentPlacementDetails[i].isCorrect) finalLevel = this.currentPlacementDetails[i].level; }
                const res = { level: finalLevel, date: new Date().toLocaleString('ar-EG'), score: this.placementScore, ielts: this.getIeltsEquivalent(finalLevel), details: this.currentPlacementDetails };
    ...
    setupGlobalEvents() { document.addEventListener('click', async (e) => { const btn = e.target.closest('[data-action]'); if (!btn) return; const { action, param, correct, total, index } = btn.dataset; if (action === 'ansQ') { this.handleAnswer(param, correct, btn); return; } if (action === 'levelTestAns') { this.handleLevelTestAnswer(param, correct, btn); return; } if (action === 'gapfillAnswer') { this.handleGapFillAnswer(btn.dataset.english); return; } if (action === 'gapfillNext') { this.handleGapFillNext(); return; } if (action === 'gapfillShowExplanation') { this.showDetailedGapFillExplanation(); return; } switch (action) { case 'masterWord': if (!this.masteredWords.includes(String(param))) { this.masteredWords.push(String(param)); this.addMasteredWordReward(param); if (this.selectedLessonId) this.grantLessonCompletionReward(this.selectedLessonId); await this.saveUserData(); } break; case 'playAudio': this.playAudio(param); break; case 'pauseAudio': this.pauseAudio(); break; case 'stopAudio': this.stopAudio(); break; case 'skipBack10': this.skipBack10(); break; case 'skipForward10': this.skipForward10(); break; case 'speedUp': this.speedUp(); this.render(); break; case 'speedDown': this.speedDown(); this.render(); break; case 'goHome': this.stopAudio(); this.currentPage = 'home'; this.selectedLessonId = null; this.isUnlockTest = false; this.viewingPlacementDetails = null; this.levelTestLevel = null; break; case 'logout': if (confirm(this.t('هل أنت متأكد من تسجيل الخروج؟', 'Are you sure you want to logout?'))) this.logout(); break; case 'selLevel': this.selectedLevel = param; this.currentPage = (param === 'custom_list') ? 'custom_lessons_view' : 'lessons'; break; case 'toggleTheme': this.toggleTheme(); break; case 'toggleLang': this.toggleLanguage(); break; case 'selLesson': this.scrollPos = window.scrollY; this.openLesson(param); break; case 'unlockWithTest': const list = window.lessonsList[this.selectedLevel] || []; const curIdx = list.findIndex(l => l.id == param); const prevId = list[curIdx - 1]?.id; if (prevId) { this.tempLessonToUnlock = param; this.selectedLessonId = prevId; this.prepareQuiz(this.getLessonDataById(prevId).terms, true); this.currentPage = 'quiz'; } break; case 'unlockWithCoins': this.unlockLessonWithCoins(param); break; case 'setPage': if (param === 'listening' && this.selectedLessonId) { if (!this.listeningUnlocked[this.selectedLessonId]) { if (!this.unlockListening(this.selectedLessonId)) return; } else this.prepareListeningQuiz(); } else if (param === 'jumble' && this.selectedLessonId) { if (!this.jumbleUnlocked[this.selectedLessonId]) { if (!this.unlockJumble(this.selectedLessonId)) return; } else this.prepareJumble(); } else if (param === 'spelling' && this.selectedLessonId) { if (!this.spellingUnlocked[this.selectedLessonId]) { if (!this.unlockSpelling(this.selectedLessonId)) return; } else this.prepareSpelling(); } else if (param === 'quiz' && this.selectedLessonId) { const lessonData = this.getLessonDataById(this.selectedLessonId); if (lessonData) this.prepareQuiz(lessonData.terms, false); } else if (param === 'profile') { this.showProfile(); return; } else if (param === 'test_history') { this.showTestHistory(); return; } else if (param === 'gapfill' && this.selectedLessonId) { if (!this.gapFillUnlocked[this.selectedLessonId]) { if (!this.unlockGapFill(this.selectedLessonId)) return; } else this.prepareGapFill(); } this.currentPage = param; this.currentCardIndex = 0; break; case 'masterWordFlash': const cardM = document.querySelector('.flashcard-container'); if (cardM) { cardM.classList.add('master-anim'); setTimeout(async () => { const wordId = String(param); if (!this.masteredWords.includes(wordId)) { this.masteredWords.push(wordId); this.addMasteredWordReward(param); if (this.selectedLessonId) this.grantLessonCompletionReward(this.selectedLessonId); await this.saveUserData(); } if (this.showAllCardsTemporary && !this.repeatAllSessionMastered.includes(wordId)) { this.repeatAllSessionMastered.push(wordId); await this.saveUserData(); } this.render(); }, 550); } return; case 'deleteWord': this.showConfirmModal(this.t('هل أنت متأكد من حذف هذه الكلمة نهائياً من بطاقاتك؟', 'Are you sure you want to permanently delete this word from your flashcards?'), async () => { const cardD = document.querySelector('.flashcard-container'); if (cardD) { cardD.classList.add('delete-anim'); setTimeout(async () => { this.hiddenFromCards.push(String(param)); await this.saveUserData(); this.render(); }, 550); } }); return; case 'speak': this.speak(param); break; case 'nextC': const lessonData = this.getCurrentLessonData(); const addedWords = this.userVocabulary.filter(v => v.lessonId == this.selectedLessonId); const allWords = lessonData ? [...lessonData.terms, ...addedWords] : []; let activeCards; if (this.showAllCardsTemporary) activeCards = allWords.filter(t => !this.hiddenFromCards.includes(String(t.id)) && !this.repeatAllSessionMastered.includes(String(t.id))); else activeCards = allWords.filter(t => !this.masteredWords.includes(String(t.id)) && !this.hiddenFromCards.includes(String(t.id))); if (activeCards.length === 0) break; const currentCard = activeCards[this.currentCardIndex]; if (currentCard && !this.skippedCards.includes(String(currentCard.id))) this.skippedCards.push(String(currentCard.id)); this.currentCardIndex++; if (this.currentCardIndex >= activeCards.length) this.currentCardIndex = 0; this.render(); break; case 'prevC': const lessonPrev = this.getCurrentLessonData(); const addedPrev = this.userVocabulary.filter(v => v.lessonId == this.selectedLessonId); const allWordsPrev = lessonPrev ? [...lessonPrev.terms, ...addedPrev] : []; let activePrev; if (this.showAllCardsTemporary) activePrev = allWordsPrev.filter(t => !this.hiddenFromCards.includes(String(t.id)) && !this.repeatAllSessionMastered.includes(String(t.id))); else activePrev = allWordsPrev.filter(t => !this.masteredWords.includes(String(t.id)) && !this.hiddenFromCards.includes(String(t.id))); if (activePrev.length === 0) break; this.currentCardIndex--; if (this.currentCardIndex < 0) this.currentCardIndex = activePrev.length - 1; this.render(); break; case 'restartCards': this.skippedCards = []; this.currentCardIndex = 0; const cardShuffle = document.querySelector('.flashcard-container'); if (cardShuffle) cardShuffle.classList.add('shuffle-anim-card'); const delay = cardShuffle ? 600 : 0; setTimeout(async () => { if (param === 'all' && this.selectedLessonId) { this.showAllCardsTemporary = true; this.repeatAllSessionMastered = []; this.currentCardIndex = 0; await this.saveUserData(); this.render(); } else if (param === 'remaining') { if (this.showAllCardsTemporary) { this.currentCardIndex = 0; await this.saveUserData(); this.render(); } else { this.showAllCardsTemporary = false; this.repeatAllSessionMastered = []; this.currentCardIndex = 0; await this.saveUserData(); this.render(); } } }, delay); this.showAd('image'); return; case 'addNewWord': this.handleNewWord(); break; case 'backToLessons': this.stopAudio(); this.currentPage = (this.selectedLevel === 'custom_list') ? 'custom_lessons_view' : 'lessons'; this.selectedLessonId = null; this.isUnlockTest = false; this.render(); setTimeout(() => window.scrollTo(0, this.scrollPos), 50); return; case 'doAuth': this.handleAuth(); return; case 'doPlacement': this.handlePlacement(param, correct, btn); return; case 'viewPlacementDetails': const record = this.placementResults[parseInt(index)]; if (record) { this.viewingPlacementDetails = record; this.currentPage = 'placement_details'; this.render(); } break; case 'viewTestHistoryDetails': this.viewTestDetails(parseInt(index)); break; case 'backFromDetails': this.viewingPlacementDetails = null; this.currentPage = 'test_history'; this.render(); break; case 'jumbleSelect': this.handleJumbleSelect(param); break; case 'jumbleRemove': this.handleJumbleRemove(param); break; case 'jumbleReset': this.handleJumbleReset(); break; case 'jumbleCheck': this.handleJumbleCheck(); break; case 'jumbleHint': this.handleJumbleHint(); break; case 'jumbleNext': this.handleJumbleNext(); break; case 'listeningAnswer': this.handleListeningAnswer(param); break; case 'spellingCheck': this.handleSpellingCheck(); break; case 'spellingNext': this.handleSpellingNext(); break; case 'startLevelTest': this.prepareLevelTest(param); break; case 'finishLevelTest': this.finishLevelTestEarly(); break; case 'watchAds': this.watchAdsForCoins(); break; case 'requestPurchase': this.requestPurchase(); break; case 'toggleCoinModal': this.toggleCoinModal(); break; case 'submitPurchase': this.submitPurchaseRequest(); break; case 'updateProfile': this.updateProfile(); break; case 'goToProfile': this.showProfile(); break; case 'showBadges': this.showBadgesModal(); break; case 'showProfileInfo': this.showProfileInfo(); break; case 'changePassword': this.changePassword(); break; case 'cancelChangePassword': this.cancelChangePassword(); break; } this.render(); }); document.addEventListener('input', (e) => { if (e.target.id === 'spellingInput') this.spellingUserAnswer = e.target.value; }); }
    ...
    render() { const app = document.getElementById('app'); if (!app) return; if (this.loadingData) { app.innerHTML = `<div style="display:flex; flex-direction:column; justify-content:center; align-items:center; height:100vh; font-family:Cairo, sans-serif;"><div class="loader" style="border:4px solid #f3f3f3; border-top:4px solid #3498db; border-radius:50%; width:40px; height:40px; animation:spin 2s linear infinite;"></div><p style="margin-top:20px; color:#666;">${this.t('جاري مزامنة بياناتك...', 'Syncing your data...')}</p><style>@keyframes spin{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}</style></div>`; return; } const lesson = this.getCurrentLessonData(); const added = this.userVocabulary.filter(v => v.lessonId == this.selectedLessonId); const allTerms = lesson ? [...lesson.terms, ...added] : []; app.innerHTML = this.getHeader() + `<div id="view">${this.getView(lesson, allTerms)}</div>`; if (this.showCoinModal) { const modalDiv = document.createElement('div'); modalDiv.className = 'modal-overlay'; modalDiv.onclick = (e) => { if (e.target === modalDiv) this.toggleCoinModal(); }; let modalContent = ''; if (this.showPurchaseForm) modalContent = `<div class="modal-content"><div class="modal-header" style="display:flex; justify-content:space-between; margin-bottom:15px;"><h3 style="margin:0;">💰 ${this.t('طلب شراء 300 لؤلؤة', 'Request 300 Pearls')}</h3><span class="close-btn" onclick="appInstance.toggleCoinModal()" style="cursor:pointer; font-size:1.5rem;">&times;</span></div><p style="text-align:center; margin-bottom:15px;">${this.t('مقابل 1 دولار أمريكي', 'For 1 USD')}</p><div class="purchase-form"><input type="text" id="purchaseName" placeholder="${this.t('الاسم الكامل', 'Full Name')}" style="width:100%; padding:10px; margin:5px 0; border-radius:8px; border:1px solid #ddd;"><input type="email" id="purchaseEmail" placeholder="${this.t('البريد الإلكتروني', 'Email')}" style="width:100%; padding:10px; margin:5px 0; border-radius:8px; border:1px solid #ddd;"><input type="tel" id="purchasePhone" placeholder="${this.t('رقم الهاتف', 'Phone Number')}" style="width:100%; padding:10px; margin:5px 0; border-radius:8px; border:1px solid #ddd;"><button class="hero-btn" data-action="submitPurchase" style="background:#10b981; width:100%; margin-top:10px;">${this.t('إرسال الطلب', 'Submit Request')}</button></div></div>`; else modalContent = `<div class="modal-content"><div class="modal-header" style="display:flex; justify-content:space-between; margin-bottom:15px;"><h3 style="margin:0;">💰 ${this.t('خيارات العملات', 'Currency Options')}</h3><span class="close-btn" onclick="appInstance.toggleCoinModal()" style="cursor:pointer; font-size:1.5rem;">&times;</span></div><div class="coin-option" onclick="appInstance.watchAdsForCoins()"><div style="display:flex; justify-content:space-between; align-items:center;"><span style="font-size:1rem;">👁️ ${this.t('مشاهدة إعلانات', 'Watch Ads')}</span><span style="background:#ffd700; padding:4px 8px; border-radius:20px;">${this.getCurrentAdReward() ? this.getCurrentAdReward().reward : 0}+</span></div></div><div class="coin-option" onclick="appInstance.requestPurchase()"><div style="display:flex; justify-content:space-between; align-items:center;"><span style="font-size:1rem;">💳 ${this.t('شراء 300 لؤلؤة', 'Buy 300 Pearls')}</span><span style="background:#ffd700; padding:4px 8px; border-radius:20px;">1$</span></div></div></div>`; modalDiv.innerHTML = modalContent; app.appendChild(modalDiv); } if (this.showProfileInfoModal) { const modalDiv = document.createElement('div'); modalDiv.className = 'modal-overlay'; modalDiv.onclick = (e) => { if (e.target === modalDiv) { this.showProfileInfoModal = false; this.showChangePasswordForm = false; this.render(); } }; let modalHtml = `<div class="modal-content" style="max-width:400px;"><div class="modal-header" style="display:flex; justify-content:space-between; margin-bottom:15px;"><h3>${this.t('معلومات المستخدم', 'User Information')}</h3><span class="close-btn" onclick="appInstance.showProfileInfoModal=false;appInstance.render()" style="cursor:pointer; font-size:1.5rem;">&times;</span></div><div style="margin-bottom:10px;"><strong>${this.t('الاسم:', 'Name:')}</strong> ${this.userProfile.name || this.userData?.name || ''}</div><div style="margin-bottom:10px;"><strong>${this.t('البريد الإلكتروني:', 'Email:')}</strong> ${this.userData?.email || ''}</div><div style="margin-bottom:10px;"><strong>${this.t('العمر:', 'Age:')}</strong> ${this.userProfile.age || this.t('غير محدد', 'Not set')}</div><div style="margin-bottom:10px;"><strong>${this.t('تاريخ الانضمام:', 'Join Date:')}</strong> ${this.userProfile.joinDate}</div><div style="margin-bottom:10px;"><strong>${this.t('المستوى في التطبيق:', 'App Level:')}</strong> ${this.userStats.level}</div><div style="margin-bottom:10px;"><strong>${this.t('مستوى اللغة:', 'Language Level:')}</strong> ${this.getEnglishLevel()}</div><div style="margin-bottom:10px;"><strong>${this.t('نقاط الخبرة (XP):', 'XP:')}</strong> ${this.userStats.xp}</div><div style="margin-bottom:10px;"><strong>${this.t('اللآلئ:', 'Pearls:')}</strong> ${this.userCoins}</div>`; if (this.showChangePasswordForm) { modalHtml += `<div style="margin-top:20px; border-top:1px solid #eee; padding-top:15px;"><h4>${this.t('تغيير كلمة المرور', 'Change Password')}</h4><input type="password" id="oldPassword" placeholder="${this.t('كلمة المرور القديمة', 'Old Password')}" style="width:100%; padding:8px; margin:5px 0; border-radius:8px; border:1px solid #ddd;"><input type="password" id="newPassword" placeholder="${this.t('كلمة المرور الجديدة', 'New Password')}" style="width:100%; padding:8px; margin:5px 0; border-radius:8px; border:1px solid #ddd;"><input type="password" id="confirmPassword" placeholder="${this.t('تأكيد كلمة المرور الجديدة', 'Confirm New Password')}" style="width:100%; padding:8px; margin:5px 0; border-radius:8px; border:1px solid #ddd;"><div style="display:flex; gap:8px; margin-top:10px;"><button class="hero-btn" data-action="changePassword" style="background:#10b981; flex:1;">${this.t('تغيير', 'Change')}</button><button class="hero-btn" data-action="cancelChangePassword" style="background:#ef4444; flex:1;">${this.t('إلغاء', 'Cancel')}</button></div></div>`; } else { modalHtml += `<div style="margin-top:20px;"><button class="hero-btn" data-action="changePassword" style="width:100%; background:#3b82f6;">${this.t('تغيير كلمة المرور', 'Change Password')}</button></div>`; } modalHtml += `</div>`; modalDiv.innerHTML = modalHtml; app.appendChild(modalDiv); } }
    getHeader() { if (this.currentPage === 'auth') return ''; let nav = ''; if (this.selectedLessonId && ['reading', 'flashcards', 'quiz', 'jumble', 'listening', 'spelling', 'gapfill'].includes(this.currentPage) && !this.isUnlockTest) { nav = `<div class="nav-menu"><button class="nav-btn ${this.currentPage === 'reading' ? 'active' : ''}" data-action="setPage" data-param="reading">${this.t('📖 النص', '📖 Text')}</button><button class="nav-btn ${this.currentPage === 'flashcards' ? 'active' : ''}" data-action="setPage" data-param="flashcards">${this.t('🎴 بطاقات', '🎴 Flashcards')}</button><button class="nav-btn ${this.currentPage === 'quiz' ? 'active' : ''}" data-action="setPage" data-param="quiz">${this.t('🧩 اختبار', '🧩 Quiz')}</button><button class="nav-btn ${this.currentPage === 'jumble' ? 'active' : ''}" data-action="setPage" data-param="jumble">${this.t('🔤 ترتيب', '🔤 Jumble')}</button><button class="nav-btn ${this.currentPage === 'listening' ? 'active' : ''}" data-action="setPage" data-param="listening">${this.t('🎧 استماع', '🎧 Listening')}</button><button class="nav-btn ${this.currentPage === 'spelling' ? 'active' : ''}" data-action="setPage" data-param="spelling">${this.t('✍️ كتابة', '✍️ Spelling')}</button><button class="nav-btn ${this.currentPage === 'gapfill' ? 'active' : ''}" data-action="setPage" data-param="gapfill">${this.t('📝 ملء فراغ', '📝 Gap Fill')}</button></div>`; } return `<header class="header"><div class="header-content"><div class="logo-container" data-action="goHome"><img src="wordwise_logo.png" alt="WordWise"><h2>WordWise</h2></div><div class="header-buttons"><button class="header-btn lang-btn" data-action="toggleLang" title="${this.t('تغيير اللغة', 'Change Language')}">${this.lang === 'ar' ? 'EN' : 'عربي'}</button><button class="header-btn" data-action="toggleTheme" title="${this.t('الوضع الليلي', 'Dark Mode')}">${this.theme === 'light' ? '🌙' : '☀️'}</button><div class="coin-display" data-action="toggleCoinModal" title="${this.t('رصيد اللآلئ', 'Pearls Balance')}"><span>💎</span> ${this.userCoins}<span style="font-size:0.9rem;">➕</span></div><button class="header-btn" data-action="goToProfile" title="${this.t('الملف الشخصي', 'Profile')}">👤</button></div></div>${nav}</header>`; }
    ...
    editLessonContent(id) { const newC = prompt(this.t("تعديل نص الموضوع:", "Edit text content:"), this.customLessons[id].content); if (newC && newC.trim()) { this.customLessons[id].content = newC.trim(); if (window.lessonsData[id]) window.lessonsData[id].content = newC.trim(); this.saveUserData(); this.render(); } }
    deleteCustomLesson(id) { this.showConfirmModal(this.t('هل أنت متأكد من حذف هذا النص نهائياً؟', 'Are you sure you want to permanently delete this text?'), () => { delete this.customLessons[id]; delete window.lessonsData[id]; this.userVocabulary = this.userVocabulary.filter(v => v.lessonId !== String(id)); this.saveUserData(); this.render(); }); }
    init() { this.addThemeStyles(); document.documentElement.setAttribute('data-theme', this.theme); if (typeof auth === 'undefined' || typeof db === 'undefined') { setTimeout(() => this.init(), 500); return; } if (!window.levels || !window.lessonsData || !window.placementBank || !window.lessonsList) { setTimeout(() => this.init(), 500); return; } this.audioCtx = new (window.AudioContext || window.webkitAudioContext)(); if (this.audioCtx.state === 'suspended') this.audioCtx.resume(); this.setupGlobalEvents(); onAuthStateChanged(auth, async (user) => { if (this.isProcessingAuth) return; if (user) { this.currentUser = user; this.userData = { name: user.displayName || '', email: user.email, uid: user.uid }; this.loadingData = true; this.isDataLoaded = false; this.canSave = false; this.currentPage = 'home'; this.render(); try { await this.loadUserData(user.uid); } catch (err) { console.error(err); } finally { this.loadingData = false; this.render(); } } else { this.currentUser = null; this.currentPage = 'auth'; this.loadingData = false; this.isDataLoaded = false; this.canSave = false; this.render(); } }); }
}
