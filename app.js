...
class App {
    ...
    getLessonsForCurrentLevel() {
        ...
        let originalLessons = window.lessonsList && window.lessonsList[this.selectedLevel] ? window.lessonsList[this.selectedLevel] : [];
    ...
    playListeningAudio(audioSrc, questionId) {
        console.log("🎵 محاولة تشغيل الصوت:", audioSrc, "للسؤال:", questionId);
        ...
        try {
            ...
            audio.play().then(() => {
                ...
                this.showCustomModal('error', '🔇', this.t(`فشل تشغيل الصوت: ${err.message}`, `Audio failed: ${err.message}`));
            });
        } catch (err) {
            console.error("❌ خطأ في إنشاء Audio:", err);
            this.showCustomModal('error', '🔇', this.t('حدث خطأ أثناء محاولة تشغيل الصوت.', 'Error playing audio.'));
        }
    }
    ...
    addDefaultReadingQuestions() {
        if (window.placementBank && Object.keys(window.placementBank).some(k => window.placementBank[k] && window.placementBank[k].length > 0)) {
            console.log("✅ placementBank موجود بالفعل، لن يتم إنشاء أسئلة افتراضية");
            return;
        }
    ...
    addDefaultListeningQuestions() {
        if (window.listeningBank && Object.keys(window.listeningBank).some(k => window.listeningBank[k] && window.listeningBank[k].length > 0)) {
            console.log("✅ listeningBank موجود بالفعل، لن يتم إنشاء أسئلة افتراضية");
            return;
        }
...
showManageGapQuestions(wordId, wordEnglish) {
    ...
    finalizeAdaptiveTest() {
        ...
        const listeningQuestions = this.adaptiveListeningHistory.map((item, idx) => ({
            id: `L${idx+1}`,
            type: 'listening',
            level: item.level,
        ...
        const readingQuestions = this.adaptiveTestHistory.map((item, idx) => ({
            id: `R${idx+1}`,
            type: 'reading',
            level: item.level,
            question: item.question,
        ...
        const allDetails = [...listeningQuestions, ...readingQuestions];
    ...
    prepareLevelTest(levelParam) {
        ...
        if (levelParam === 'beginner') { levelName = 'beginner'; lessonIds = window.lessonsList && window.lessonsList['beginner'] ? window.lessonsList['beginner'].map(l => l.id) : []; }
        else if (levelParam === 'intermediate') { levelName = 'intermediate'; lessonIds = window.lessonsList && window.lessonsList['intermediate'] ? window.lessonsList['intermediate'].map(l => l.id) : []; }
        else if (levelParam === 'advanced') { levelName = 'advanced'; lessonIds = window.lessonsList && window.lessonsList['advanced'] ? window.lessonsList['advanced'].map(l => l.id) : []; }
        else return;
...
async prepareGapFill() {
    ...
    getLessonDataById(id) { if (window.lessonsData && window.lessonsData[id]) return window.lessonsData[id]; if (this.generatedLessons[id]) return this.generatedLessons[id]; return null; }
...
if (action === 'catAnswer') {
    ...
    return;
            ...
            switch (action) {
                ...
                case 'goHome': this.stopAudio(); this.currentPage = 'home'; this.selectedLessonId = null; this.isUnlockTest = false; this.viewingPlacementDetails = null; this.levelTestLevel = null; this.adaptiveTestActive = false; this.adaptiveListeningActive = false; this.catActive = false; break;
                ...
                case 'unlockWithTest': const list = window.lessonsList && window.lessonsList[this.selectedLevel] ? window.lessonsList[this.selectedLevel] : []; const curIdx = list.findIndex(l => l.id == param); const prevId = list[curIdx - 1]?.id; if (prevId) { this.tempLessonToUnlock = param; this.selectedLessonId = prevId; this.prepareQuiz(this.getLessonDataById(prevId).terms, true); this.currentPage = 'quiz'; } break;
...
checkCatAnswer(selectedAnswer, correctAnswer, btnElement) {
    ...
    showCatHint() {
        ...
        if (!currentQ || !currentQ.hint) {
            alert(this.t('لا يوجد تلميح لهذا السؤال', 'No hint for this question'));
            return;
        }
        const hintDiv = document.getElementById('catHint');
        if (hintDiv) {
            hintDiv.innerHTML = `💡 ${this.escapeHtml(currentQ.hint)}`;
...
<button class="hero-btn" data-action="showCatHint" style="background:#f59e0b; padding:6px 12px; font-size:0.9rem;">💡 ${this.t('تلميح', 'Hint')}</button>
</div>
...
if (this.currentPage === 'flashcards') {
    ...
    // الوجه الأمامي: الكلمة + نوعها (إن وجد)
    let frontContent = `<h1>${this.escapeHtml(t.english || '')}</h1>`;
    if (t.pos) {
        frontContent += `<div style="font-size:0.8rem; margin-top:8px; color:rgba(255,255,255,0.7);">${this.escapeHtml(t.pos)}</div>`;
    }

    // الوجه الخلفي: المعنى + مثال + ترجمة المثال (إن وجد)
    let backContent = `<h1>${this.escapeHtml(t.arabic || '')}</h1>`;
    if (t.example) {
        backContent += `<div style="font-size:0.8rem; margin-top:12px; text-align:left; direction:ltr;"><strong>Example:</strong> ${this.escapeHtml(t.example)}</div>`;
    }
    if (t.example_ar) {
        backContent += `<div style="font-size:0.8rem; margin-top:6px; text-align:right; direction:rtl;"><strong>مثال:</strong> ${this.escapeHtml(t.example_ar)}</div>`;
    }
    ...
    return `<main class="main-content">
        ...
        <div id="editArea_${t.id}" style="display:none; margin-top:20px; background:#f8fafc; padding:15px; border-radius:12px; border:1px solid #cbd5e1;">
            <input type="text" id="editEng_${t.id}" value="${this.escapeHtml(t.english || '')}" style="width:100%; padding:10px; margin-bottom:10px; border-radius:8px; border:1px solid #ddd; text-align:center;">
            <input type="text" id="editArb_${t.id}" value="${this.escapeHtml(t.arabic || '')}" style="width:100%; padding:10px; margin-bottom:10px; border-radius:8px; border:1px solid #ddd; text-align:center; direction:rtl;">
        ...
        <div class="card-controls-row" style="margin-top:20px;">
            <button class="hero-btn" data-action="speak" data-param="${this.escapeHtml(t.english)}" style="background:#6366f1;">🔊</button>
        ...
        <!-- أزرار إضافة وإدارة أسئلة GapFill -->
        <div style="display: flex; gap: 8px; margin: 10px 0; justify-content: center;">
            <button class="hero-btn" onclick="appInstance.showAddGapFillQuestion('${t.id}', '${this.escapeHtml(t.english)}', '${this.escapeHtml(t.arabic)}')" style="background:#8b5cf6; padding: 6px 12px; font-size: 0.75rem;">➕ ${this.t('إضافة سؤال', 'Add Q')}</button>
            <button class="hero-btn" onclick="appInstance.showManageGapQuestions('${t.id}', '${this.escapeHtml(t.english)}')" style="background:#f59e0b; padding: 6px 12px; font-size: 0.75rem;">📋 ${this.t('إدارة', 'Manage')}</button>
        </div>
...
editLessonContent(id) { 
    ...
    init() { 
        ...
        if (!window.levels || !window.lessonsData || !window.placementBank || !window.lessonsList) { 
            ...
            if (!window.placementBank) window.placementBank = {};
            if (!window.lessonsList) window.lessonsList = {};
        } 
        this.audioCtx = null;
