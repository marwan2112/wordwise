class App {
    constructor() {
        ...
        this.adaptiveTestCurrentLevel = 'A2'; // تم التعديل: يبدأ من A2 بدلاً من B1
    ...
    showAd(type, callback) {
        ...
        modalDiv.innerHTML = '<div class="modal-content" style="text-align:center;">' +
    ...
    showCustomModal(type, icon, message, onConfirm = null, onCancel = null) {
        ...
        if (type === 'question') {
            modalDiv.innerHTML = '<div class="modal-content result-modal">' +
                ...
                '<button class="hero-btn" id="modalConfirmYesBtn" style="background:#10b981;">' + this.t('نعم', 'Yes') + '</button>' +
                '<button class="hero-btn" id="modalConfirmNoBtn" style="background:#ef4444;">' + this.t('لا', 'No') + '</button>' +
                '</div></div>';
            document.body.appendChild(modalDiv);
    ...
    showCoinPurchaseModal(price, onConfirm, onCancel = null) {
        ...
        modalDiv.className = 'modal-overlay';
        modalDiv.onclick = (e) => { if (e.target === modalDiv) modalDiv.remove(); if (onCancel) onCancel(); };
        modalDiv.innerHTML = '<div class="modal-content result-modal" style="text-align:center;">' +
            '<div class="result-icon" style="font-size:2.5rem;">💎</div>' +
            '<div class="result-message" style="font-size:1rem; margin-bottom:15px;">' + this.t('هل تريد فتح هذه الميزة بـ ' + price + ' لؤلؤة؟', 'Do you want to unlock this feature for ' + price + ' pearls?') + '</div>' +
            '<div style="display:flex; gap:12px; justify-content:center;">' +
    ...
    startAdaptiveLevelTest() {
        ...
        this.loadAdaptiveQuestionSet('A2', 5); // يبدأ من A2
    ...
    finalizeAdaptiveTest() {
        ...
        const result = {
            ...
            levelStats: levelStatsForDisplay,
        ...
        try {
            ...
            console.log("✅ تم حفظ الاختبار في localStorage كنسخة احتياطية");
        ...
        if (this.currentUser) {
            ...
            this.saveUserData().then(() => {
                ...
                console.error("❌ فشل حفظ Firebase:", err);
            });
        } else {
            console.warn("⚠️ المستخدم غير مسجل، تم الحفظ محلياً فقط");
        }
    ...
    showAdaptiveResult() {
        ...
        if (lastResult.levelStats && lastResult.levelStats.length > 0) {
            ...
            for (let stat of lastResult.levelStats) {
                ...
                let levelColor = percent >= 70 ? '#10b981' : (percent >= 50 ? '#f59e0b' : '#ef4444');
